...
// Read register — wait at negedge so FF has settled after last posedge
...
// go(n): reset + run n cycles + wait half-cycle past last posedge
// The @(negedge clk) ensures we sample registers AFTER the last FF update
task go(input int n);
...
task test_branches();
    ...
    // Stores to separate memory region (high addresses)
    ...
    go(100);
    // Read results from memory — avoids hierarchical ref timing
    // Debug: show raw memory contents and register values
...
task test_csr();
    ...
    // Store all to high memory at 0x3000
...
task test_timer_irq();
    ...
    @(negedge clk);
    // mcause at 0x4000 (stored first), sentinel 0xBEF at 0x4008 (stored second)
    chk("mcause MTI",
