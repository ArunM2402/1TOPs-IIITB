...
module mmu_sv39 (
    ...
    input  logic        tlb_flush,    // SFENCE.VMA — flush TLB
...
logic [63:0] tlb_va_tag [TLB_ENTRIES]; // VA[63:12]
logic [63:0] tlb_pa     [TLB_ENTRIES]; // PA[63:12]
logic        tlb_valid  [TLB_ENTRIES];
logic [1:0]  tlb_priv   [TLB_ENTRIES];

logic [TLB_IDX_W-1:0] tlb_idx;
assign tlb_idx = va[TLB_IDX_W+11:12]; // index into TLB

logic tlb_hit;
logic [63:0] tlb_pa_out;
assign tlb_hit    = tlb_valid[tlb_idx] &&
                    (tlb_va_tag[tlb_idx] == {satp[43:0], va[63:12]}) &&
                    (tlb_priv[tlb_idx] == priv_mode);
assign tlb_pa_out = {tlb_pa[tlb_idx][63:12], va[11:0]};

// TLB flush and fill
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n || tlb_flush) begin
        for (int i=0;i<TLB_ENTRIES;i++) tlb_valid[i] <= 1'b0;
    end else if (state == DONE_OK) begin
        // Fill TLB on successful walk
        tlb_valid  [tlb_idx] <= 1'b1;
        tlb_va_tag [tlb_idx] <= {satp[43:0], va[63:12]};
        tlb_pa     [tlb_idx] <= {10'h0, pte[53:10]};
        tlb_priv   [tlb_idx] <= priv_mode;
    end
end
    ...
    if (!rst_n) begin
        ...
        if (req && !mmu_active) begin
            ...
            case (state)
                ...
                DONE_OK: begin
                    // Permission checks (no local logic var — XSIM forbids in always_ff)
                    if (
                        ((priv_mode == 2'b00) && !pte_u) ||        // U accessing S page
                        ((priv_mode == 2'b01) && pte_u && !mstatus_sum) || // S accessing U without SUM
                        (is_fetch  && !pte_x) ||                   // no execute perm
                        (!is_fetch && !is_write && !pte_r && !(mstatus_mxr && pte_x)) || // no read
                        (is_write  && (!pte_w || !pte_d)) ||       // no write or dirty
                        (!pte_a)                                    // A bit not set
                    ) begin
                        page_fault <= 1'b1;
                    end else begin
